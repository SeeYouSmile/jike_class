...
public class UserA2BServiceImpl implements UserA2BService {
    ...
    public boolean transfer(TransferDTO transferDTO) {
        ...
        //超过余额返回失败
        if(Transfer.CurrentType.USD.equals(type)&&transferDTO.getMoney()>user.getUsd()){
            throw new HmilyRuntimeException("美元余额不足");
        }else if(Transfer.CurrentType.CNY.equals(type)&&transferDTO.getMoney()>user.getCny()){
            throw new HmilyRuntimeException("人民币余额不足");
        }
        //锁定金额
        if(Transfer.CurrentType.USD.equals(type)){
        ...
        }else if(Transfer.CurrentType.CNY.equals(type)){
    ...
    public boolean confirm(TransferDTO transferDTO) {
        System.out.println("confirm:"+transferDTO.toString());
        User user = userMapper.selectByUserId(transferDTO.getUser_id_from());
        String type = transferDTO.getType();
        //取消锁定，添加余额
        if(Transfer.CurrentType.USD.equals(type)){
    ...
    public boolean cancel(TransferDTO transferDTO) {
        System.out.println("cancel:"+transferDTO.toString());
        User user = userMapper.selectByUserId(transferDTO.getUser_id_from());
        String type = transferDTO.getType();
        //返回余额，取消锁定
        if(Transfer.CurrentType.USD.equals(type)){
